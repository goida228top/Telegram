...
const RemotePeerComponent: React.FC<{
    ...
    useEffect(() => {
        if (videoRef.current && peerData.stream) {
            ...
            // Explicitly call play() to work around strict autoplay policies.
            const playPromise = videoRef.current.play();
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    console.error("Audio playback failed for remote peer:", error);
                });
            }
        }
    }, [peerData.stream]);
...
/**
 * Robustly gets user media, trying different constraints and falling back gracefully.
 * @param prefersVideo - Whether the user wants to start with video.
 * @returns An object with the media stream and a flag indicating if video is enabled.
 */
const getMediaStream = async (prefersVideo: boolean): Promise<{ stream: MediaStream; videoEnabled: boolean }> => {
    ...
    const videoConstraintsPresets = [
        { width: { ideal: 1280 }, height: { ideal: 720 }, facingMode: 'user' }, // HD
        { width: { ideal: 640 }, height: { ideal: 480 }, facingMode: 'user' },  // VGA
        { facingMode: 'user' }, // Any front camera
        true // Generic video request
    ];
    ...
    for (const constraints of videoConstraintsPresets) {
        try {
            ...
            console.log('Successfully got video stream with constraints:', constraints);
            return { stream, videoEnabled: true };
        } catch (err: any) {
            lastError = err;
            console.warn(`getUserMedia with constraints ${JSON.stringify(constraints)} failed:`, err.name);
            // If it's not an over-constrained error, it's likely a permission or hardware issue, so we stop trying.
    ...
    // If all video attempts failed, try for audio-only as a fallback.
    ...
    try {
        ...
        // Throw the most relevant error (the last video error or the final audio error)
